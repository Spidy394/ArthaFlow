...
registerRoute(
  ...
  ({ request, url }: { request: Request; url: URL }) => {
    // If this is a URL that starts with /api, return false
    if (url.pathname.startsWith('/api/')) {
      return false;
    }
    
    // If this looks like a URL for a resource, because it contains a file extension, return false
...
registerRoute(
  ...
  new CacheFirst({
    cacheName: 'images',
    plugins: [
      // Don't keep more than 50 items, and expire them after 30 days
...
// For example, you might want to handle offline fallbacks
